...
/**
 * {@link WebSocketProxyClient}
 *
 * <p>Class WebSocketProxyClient Definition
 *
...
public class WebSocketProxyClient {
  ...
  private final ConcurrentHashMap<String, ConcurrentHashSet<ConnectionInfo>> mConnectedData;
  private final ConcurrentHashMap<String, ConcurrentHashSet<ConnectionInfo>> mConnectingData;
  ...
  private static class ConnectionInfo {
    ...
    public boolean equals(Object o) {
      if (o instanceof ConnectionInfo) {
        if (connectionId.equals(((ConnectionInfo) o).connectionId)) {
          return uri.equals(((ConnectionInfo) o).uri);
        }
      }
      return false;
    }
  ...
  public WebSocketProxyClient() {
    log.info("WebSocketProxyClient()");
  ...
  public String connect(ArrayList<String> uriList, MultiMap headers) {
    ...
    ConcurrentHashSet<ConnectionInfo> connectingInfoSet = new ConcurrentHashSet<>();
  ...
  private void reconnect() {
    mConnectingData
        .values()
        .forEach(connectionInfoSet -> connectionInfoSet.forEach(this::startToConnect));
  }
  ...
  private void startToConnect(final ConnectionInfo connectionInfo) {
    ...
    if (log.isInfoEnabled()) {
      log.info("------------------------------------------------------------------------");
      log.info("WebSocket Request Headers (Proxy)");
      log.info("------------------------------------------------------------------------");
      ...
      public void handle(WebSocket websocket) {
        ...
        websocket.frameHandler(new Handler<WebSocketFrame>() {
          ...
          public void handle(WebSocketFrame frame) {
            log.trace("Frame Data : " + frame.textData());
          }
        });
        ...
        websocket.closeHandler(new Handler<Void>() {
          @Override
          public void handle(Void event) {
            log.info("Closed # " + connectionInfo.uri);
            // Reconnect
            synchronized (mConnectionLock) {
              ConcurrentHashSet<ConnectionInfo> connectedInfoSet = mConnectedData.get(connectionId);
              if (connectedInfoSet != null) {
        ...
        websocket.exceptionHandler(new Handler<Throwable>() {
          ...
          public void handle(Throwable event) {
            log.error("Exception # " + connectionInfo.uri);
...
//              mConnectedData.put(connectionId, connectedInfoSet);
//            } else {
//              if (connectedInfoSet.contains(connectionInfo)) {
//                websocket.close();
//                return;
//              }
//            }
...
//
//          // Handler (Frame)
//          websocket.frameHandler(frame -> log.info("Frame Data : " + frame.textData()));
//
//          // Handler (End)
//          websocket.drainHandler(eventUnused -> log.info("End"));
//
//          // Handler (End)
//          websocket.endHandler(eventUnused -> log.info("End"));
//
//          // Handler (Close)
...
//              eventThrowable -> {
//                log.error("Exception # " + connectionInfo.uri);
//                log.error("Exception # " + eventThrowable);
//              });
//        });
  }
}
