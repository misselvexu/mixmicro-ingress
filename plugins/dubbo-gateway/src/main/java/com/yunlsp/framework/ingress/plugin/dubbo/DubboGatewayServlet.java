...
public class DubboGatewayServlet extends HttpServletBean {
  ...
  /** 泛化服务缓存 已知的 dubbo 2.7.3 2.7.4.1 版本需要 dubbo 2.6.* 2.7.5 2.7.6不需要 当前使用dubbo版本 2.7.4.1 */
  ...
  public DubboGatewayServlet(
      DubboServiceMetadataRepository repository,
      DubboGenericServiceFactory serviceFactory,
      Cache<DubboRestServiceMetadata, GenericService> restServiceCache) {
    this.repository = repository;
    this.serviceFactory = serviceFactory;
    this.restServiceCache = restServiceCache;
  ...
  public void service(HttpServletRequest request, HttpServletResponse response) throws IOException {
    ...
    // TODO: enabled async servlet context support
    ...
    try {
      ...
      // invoke remote
      Object result = genericService.$invoke(dubboRestMethodMetadata.getMethod().getName(), parameterTypes, args);
      ...
      log.warn("dubbo generic invoke exception,rest path:[{}],param types:[{}],params:[{}],ex:{}.", restPath, parameterTypes, args, e);
    }
  }
  ...
  private static final String GENERIC_POJO_FLAG = "class";

  private void buildResponse(HttpServletResponse response, Object res) throws IOException {
    response.setCharacterEncoding("UTF-8");
    if (res instanceof Map) {
      Map<?,?> resMap = (Map<?,?>) res;
      if (resMap.get(GENERIC_POJO_FLAG) != null) {
        resMap.remove(GENERIC_POJO_FLAG);
      }
  ...
  private synchronized GenericService genericService(DubboRestServiceMetadata dubboRestServiceMetadata) {
    ...
    if (genericService == null) {
      genericService = serviceFactory.create(dubboRestServiceMetadata, dubboTranslatedAttributes);
      restServiceCache.put(dubboRestServiceMetadata, genericService);
      log.info("cache miss or init,cache size:[{}],cache stat:[{}],service url:[{}]", restServiceCache.size(), restServiceCache.stats(), dubboRestServiceMetadata.getServiceRestMetadata().getUrl());
    }
  ...
  private Object paramSwitch(String value, String valueType) {
    ...
    if (String.class.getName().equals(valueType)) return String.valueOf(value);
    if (Integer.class.getName().equals(valueType) || int.class.getName().equals(valueType)) return Integer.valueOf(value);
    if (Byte.class.getName().equals(valueType) || byte.class.getName().equals(valueType)) return Byte.valueOf(value);
    if (Long.class.getName().equals(valueType) || long.class.getName().equals(valueType)) return Long.valueOf(value);
    if (Double.class.getName().equals(valueType) || double.class.getName().equals(valueType)) return Double.valueOf(value);
    if (Float.class.getName().equals(valueType) || float.class.getName().equals(valueType)) return Float.valueOf(value);
    if (Character.class.getName().equals(valueType) || char.class.getName().equals(valueType)) return value;
    if (Short.class.getName().equals(valueType) || short.class.getName().equals(valueType)) return Short.valueOf(value);
    if (Boolean.class.getName().equals(valueType)  || boolean.class.getName().equals(valueType)) return Boolean.valueOf(value);
    if (List.class.getName().equals(valueType)) return new ArrayList<>(Collections.singletonList(value));
    // default value
    return value;
  }
